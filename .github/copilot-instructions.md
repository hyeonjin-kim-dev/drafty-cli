...
`drafty [태그...]` → 시스템 에디터 → Supabase 저장. 인증은 이메일 OTP.
...
src/
  ...
  commands/          # 각 명령어 파일 (capture, login, list, show, edit, logout, whoami)
  lib/               # 핵심 모듈 (auth, supabase, errors, notes, session-store, config, editor, prompt, parse-tags)
...
- **파일명**: `kebab-case` (예: `session-store.ts`)
- **에러**: `DraftyError` 또는 `wrapSupabaseError()` 사용, `new Error()` 금지
- **인증**: `requireAuthenticatedSession()` → `createNotesClient(session)` 패턴 준수
